	...
	public List<Tag> createTag(List<String> tags) {
		...
		for (int i = 0; i < tags.size(); i++) { // For para agregar o descartar cada tag según exista o no.
			...
			for (int j = 0; j < allTags.size(); j++) {
				if (name.equals(allTags.get(j).getName())) { // Si el tag ya existe en la lista de tags, entonces agrega
																// el tag que coincida con el nombre y luego lo cambia
																// el string a "exists"
			...
			if (!name.equals("exists")) { // si el nombre no es igual a exists, entonces lo crea, lo agrega a la lista
											// de tags que se van a retornar y lo guarda en la db
